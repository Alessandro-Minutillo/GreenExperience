from util.singleton import Singleton
import sqlite3


# Connessione singleton al database SQLite dell'applicazione.
@Singleton
class DB_Connector():

    # Apre la connessione al database e abilita la modalità WAL.
    def __init__(self):

        try:
            self.sqliteConnection = sqlite3.connect('db/database.db')
            self.sqliteConnection.row_factory = sqlite3.Row
            self.sqliteConnection.execute("PRAGMA journal_mode = WAL")

        except sqlite3.Error as error:
            print("Error while connecting to sqlite", error)

    # Restituisce un cursore sulla connessione.
    def get_cursor(self):
        return self.sqliteConnection.cursor()

    # Restituisce l'oggetto connessione.
    def get_connection(self):
        return self.sqliteConnection

    # Chiude cursore e connessione al database.
    def close_connection(self):
        if self.sqliteConnection:
                self.sqliteConnection.cursor().close()
                self.sqliteConnection.close()
